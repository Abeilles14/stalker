...
from controller_hid import XboxControllerHID
...
def read_controller():
    while True:
        if stalkerStatus['controlMode'] == ControlMode.MANUAL.value:
            (is_command_new, command) = controller_input.read(
                stalkerStatus['mission'])
...
# Controller
if WIRELESS == True:
    controller_input = XboxControllerHID()
else:
    controller_input = XboxController()
